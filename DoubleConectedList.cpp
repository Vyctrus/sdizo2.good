...
bool DoubleConectedList::searchNumbGraphV2(int var1, int var2)
{
	listElement* iter = front;
	for (int i = 0; i < size; i++) {
		if (iter->valueOfEdge == var2 && iter->value==var1) {
			//std::cout << numbWanted << " -liczba jest w strukturze\n";
			return true;
		}
		iter = iter->next;
...
int DoubleConectedList::searchNumbGetValue(int var1)
{	
	listElement* iter = front;
	for (int i = 0; i < size; i++) {
		if (iter->value == var1) {
...
int DoubleConectedList::getSize()
{
	return size;
}
